...
new_model = "Llama-2-7b-ChatQA-Generator-PubMedQA"
lora_r = 64
lora_alpha = 16
lora_dropout = 0.1
use_4bit = True
...
def formatting_prompts_func(example):
    ...
    for i in range(len(example['question'])):
        text = f"### Question: {example['question'][i]}\n ### Context: {example['context'][i]}\n  ### Levels of Evidence:{example['loe'][i]}  ### Answer:{example['answer'][i]}"
...
config.save_pretrained("Llama-2-7b-ChatQA-Generator-PubMedQA")
